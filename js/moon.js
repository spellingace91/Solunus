function calculateMoonTimes(data, latitude, longitude) {
    var date = moment.unix(time),
        mj = mjd( date.date(), date.month() + 1, date.year(), 0.0 );
    
    find_moonrise_set( mj, (data.dstOffset + data.rawOffset) / 3600, longitude, latitude);
}
	...
	date = mjd - tz/24;
		...
		while(hour < 25 && (sett == false || rise == false)) {
			...
			if (rise == true || sett == true ) {
				if (rise == true) {
    				 outstring += " " + formatTimes(utrise) /* hrsmin(utrise) */;
    				 console.log(formatTimes(utrise));
                }
				else outstring += " ----";
				if (sett == true) {
    				outstring += " " + formatTimes(utset) /* hrsmin(utset) */;
    				
    				// if the moonset occurs on the following day, approximate the time by adding 40 minutes
    				if (utset < utrise) {
        				utset += .66666;
    				}
    				
    				console.log(formatTimes(utset));
                }
				else outstring += " ----";
				}
			else {
            ...
            $(".moon .up h4").html(formatTimes(utrise));
            $(".moon .down h4").html(formatTimes(utset));
